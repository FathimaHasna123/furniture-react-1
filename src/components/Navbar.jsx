...
function Navbar() {
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full h-8 bg-[#1346af] flex items-center justify-center">
        <h1 className="text-sm md:text-base text-white font-bold">
  ...
  <Link to="/heart">
    <FaRegHeart className="h-5 w-5 text-gray-800 hover:text-blue-800" />
  </Link>
  <Link to="/shopping">
        ...
        <li key={path}>
          <Link to={path}
            className={`transition hover:text-blue-800 ${
              pathname === path ? 'text-blue-800 font-semibold' : 'text-gray-800' }`}>
            {label}
          </Link>
        </li>
      ))}
    ...
    <div className="hidden md:flex items-center space-x-5">
      <Link to="/search"><IoSearch className="h-5 w-5 hover:text-blue-800" /></Link>
...
<div className={`fixed inset-0 z-[3000] transition duration-300 ${isMenuOpen ? 'block' : 'hidden'}`}>
  ...
  <div className="absolute top-0 left-0 w-64 h-full bg-white shadow-lg p-6 overflow-y-auto transition-transform transform translate-x-0">
  ...
  {[
    ...
    <li key={path}>
      <Link to={path}
        onClick={closeMenu}
        className={`block w-full py-3 px-2 text-left transition hover:text-blue-800 ${
          pathname === path ? 'text-blue-800 font-semibold ' : 'text-gray-900'}`}>
        {label}
      </Link>
    </li>
  ))}
